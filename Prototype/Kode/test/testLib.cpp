...
void Library::plot2(unsigned short data1, unsigned short data2 )
{
	unsigned short buffer[20]; //Buffer needed to store data packet for transmission
	unsigned short pktSize;

  buffer[0] = 0xCDAB;             		//SimPlot packet header. Indicates start of data packet
  buffer[1] = 2*sizeof(unsigned short);	//Size of data in bytes. Does not include the header and size fields
  buffer[2] = data1;
  buffer[3] = data2;



  pktSize = 2 + 2 + (2*sizeof(unsigned short));//(6*sizeof(int)); //Header bytes + size field bytes + data
